<x-app-layout>
    ...
    <body>
        ...
        <!-- End Navigation Bar -->
        <!-- Sausages Products -->
        <div class="container my-5 p-4 title-product text-center">
            <h1 class="my-4">SAUSAGES</h1>
            <div class="row mx-5 text-center">
                @foreach ($sausages as $sausage)
                    <div class="col mx-5">
                        <div class="card mb-3">
                            <img src="/storage/images/{{$sausage->product_image}}" class="card-img-top" alt="...">
                            <div class="card-body">
                                <h6 class="card-title mb-2">{{$sausage->product_name}}</h6>
                                <p class="card-text mb-1">&#8369; {{$sausage->product_price}}</p>
                                <a class="btn btn-outline-light btn-cart text-light" style="background-color: #000000;" href="{{ route('add.to.cart', $sausage->product_id) }}">ADD TO CART</a>
                            </div>
                        </div>
                    </div>
                @endforeach
